...
for jj in range(len(allzsb.ix[0])-3):
...
for jk in range(len(allzsb[0])):
	...
	plt.title('Extracted C$_3$ vs. K$_0$ for each Temp. (Br K) pb rfix ')
	plt.show()
	plt.legend()
...
plt.figure()
for jk in range(len(allzsb[0])):
	plt.plot(allzsb.ix[jk],c3_all_br_r.ix[jk],label=str(temps[jk])+'K',marker='x')
	plt.xlabel('k$_0$ crossing ($\AA^{-1}$)')
	plt.ylabel('Extracted C$_3$')
...
for jk in range(len(allzsb[0])):
	...
	plt.plot(allzsb.ix[jk],c3_all_nor.ix[jk],label=str(temps[jk])+'K',marker='+')
	plt.xlabel('k$_0$ crossing ($\AA^{-1}$)')
	plt.ylabel('Extracted C$_3$')
