...
for jj in range(len(allzsb.ix[0])-4):
	for hh in range(len(allzsb[0])):
		...
		if jj>1 and jj<len(allzsb.ix[0])-4:
			D_matrix[jj][hh]=0.5*(allzsb[jj+1][hh]-allzsb[jj-1][hh])
			ratmat[jj][hh]=del_matrix[jj][hh]/D_matrix[jj][hh]
			kbratmat[jj][hh]=-1.*(3/2.)*(1/scaleisobk[jj]**2)*(drdt-scaleisobk[jj]*sigslope*del_matrix[jj][hh]/D_matrix[jj][hh])
		print(allzsb[jj][hh]-allzsb[jj][0])


#ratmat.dropna(axis=1,inplace=True)
t_avg_ratmat=kbratmat[3]
#outrat=t_avg_ratmat.apply(inplace=True, lambda x: -1.*(x))
plt.figure()

plt.plot(temps,t_avg_ratmat,label='extarct')
plt.plot(c3s['t'],c3s['slope'])
