...
#datals=['pb_110Kff.dat','pb_145Kff.dat','pb_160Kff.dat','br_185Kff.dat','br_200Kff.dat','br_232Kff.dat','br_243Kff.dat','br_275Kff.dat','br_300Kff.dat']
...
fk_datain='f_keep_pb-br_mapbr.dat'
epsilon=.000003
...
'''
#scaleisobk=[5.76097,6.36702,6.96469,7.56337,8.16744,8.76151]
#scaleisobchi=[.0568144,	-0.0500456,.0515359,-.0502997,.0426314,-.0396179]

scaleisobk=[6.36702,6.96469,7.56337,8.16744,8.76151]
for pp in range(len(scaleisobk)):
	if pp%2==0:
		scaleisobk[pp]=.998*scaleisobk[pp]
	else:
		scaleisobk[pp]=.998*scaleisobk[pp]


scaleisobchi=[-0.0500456,.0515359,-.0502997,.0426314,-.0396179]
...
class KData:
	...
	def reader(self):
		...
		return df, kz_cross, int_func, smoothdf_scaled
...
sigs=pd.read_csv('pb_sigsqrs.dat', sep='\s')
sigs=sigs.dropna(axis=1)
sigs.columns=['t','sig','unc']

linear_regressor = LinearRegression()  # create object for the class
bou=linear_regressor.fit(sigs['t'].reshape(-1,1), sigs['sig'].reshape(-1,1))
...
##### c3 fit reader
c3s=pd.read_csv('pb_c3fit.dat', sep='\s+')
...
# hh is i (ith temp)
for jj in range(len(allzsb.ix[0])-2):
...
c3plot_offset=c3plot_offset.apply(lambda x: x - (c3plot_offset[1]-0.000127264))
plt.figure()
plt.plot(pd.Series(temps[1:]),c3plot_offset[1:],label='pbextarct', marker='*')
plt.plot(temps,t_avg_ratmat)
plt.plot(c3s['t'],c3s['c33'])
#c3plot_offset['t']=pd.Series(temps)
c3plot_offset.to_csv('isob_pb_c3_minuspt2pct.dat', sep='\t')
plt.legend()
